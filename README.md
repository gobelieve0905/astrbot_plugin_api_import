...
4. 列表中可搜索、编辑、启停和删除。删除有页面内确认，保存和删除都不会调用目标 API。
...
不支持的选项、文件引用、管道和多条命令会明确报错，不静默跳过。cURL 草稿保留固定请求值，不猜测业务参数含义；导入后请按需改为模型参数。暂不支持 OpenAPI 导入。
...
本版提供页面管理、表单/cURL/JSON 接入、工具注册、HTTP 执行、结果提取、保存和试调用。尚不包含 OpenAPI 导入、自动分页、异步报表轮询、文件上传或工作流。返回的数据仅代表一次响应，不承诺已拉完全部分页。
