...
`document_import.py` 从 HTML 提取正文、代码和表格，丢弃脚本样式；使用默认 AstrBot 文本模型生成带原文依据的内部规范草稿。Key 在进入模型前移除，模型无 tools、无历史；模型输出经过结构/路径依据校验及既有同源转换后，仍须由页面选择启用。标准文档不进入模型。网页上限 8 MB、正文 60000 字符、模型 90 秒、手动文档流程 120 秒；不执行网页脚本或自动重定向。
