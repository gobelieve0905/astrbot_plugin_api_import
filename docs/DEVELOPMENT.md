...
56 项单元测试通过，新增普通网页与粘贴文本、凭据隔离、来源依据、模型故障、超限和跨域检查；浏览器新增普通文档入口、草稿原文和参数说明、默认停用保存流程。真实框架隔离检查使用假模型校验 provider 调用参数与页面响应，不发真实模型请求或业务请求。
...
只读 URL 查询可生成 x-method-inferred GET 草稿，界面显式提示确认，默认停用，推断说明保留在定义描述中。其他方法禁止推断，需在原文方法证据中出现明确方法名称。单元和浏览器检查覆盖该差异。
