...
function documentationLink(url, label) {
  try {
    ...
    address.title = '点击选中完整网址，手动复制到浏览器查看';
    box.append(address);
    return box;
  } catch { return el('span', label); }
}
function descriptionText(text, className = 'muted', info = null) {
  const section = el('div', undefined, 'documentation-section');
  const urls = new Map();
  const add = (url, label) => {
    try {
      const parsed = new URL(url);
...
function operationCard(item) {
  ...
  details.append(el('summary', '接口详情'), el('p', item.request.url, 'endpoint'), descriptionText(item.description, 'muted', info));
...
function renderPlatform() {
  ...
  for (const operation of platform?.operations || []) {
    ...
    const detail = el('details'); detail.append(el('summary', '查看接口与参数'), el('p', `${operation.method} ${operation.path}`, 'hint'), descriptionText(operation.description, 'hint', operation));
